...
if (existsSync("package.json")) {
  try {
    ...
    if (pkg.scripts?.["content:upcoming"] !== "node scripts/generate-upcoming-actions.mjs") failures.push("package.json is missing the content upcoming actions script.");
    if (pkg.scripts?.["content:performance"] !== "node scripts/generate-performance-log.mjs") failures.push("package.json is missing the content performance log script.");
    if (pkg.scripts?.["content:review"] !== "node scripts/generate-post-publication-review.mjs") failures.push("package.json is missing the post-publication review script.");
    if (pkg.scripts?.["content:links"] !== "node scripts/generate-tracked-links.mjs") failures.push("package.json is missing the content tracked links script.");
...
if (existsSync("scripts/generate-upcoming-actions.mjs")) {
  const upcomingGenerator = read("scripts/generate-upcoming-actions.mjs");
  for (const marker of ["content-automation/UPCOMING_ACTIONS.md", "content-automation/campaigns", "Upcoming Queue", "Date Summary", "Preparation Log", "Asia/Seoul", "Generated upcoming actions"]) {
    if (!upcomingGenerator.includes(marker)) failures.push(`scripts/generate-upcoming-actions.mjs is missing marker: ${marker}`);
  }
}
...
if (existsSync("scripts/generate-tracked-links.mjs")) {
  const trackedLinksGenerator = read("scripts/generate-tracked-links.mjs");
  for (const marker of ["content-automation/TRACKED_LINKS.md", "content-automation/TRACKED_LINKS.csv", "utm_source", "utm_medium", "utm_campaign", "utm_content", "Generated tracked links"]) {
    if (!trackedLinksGenerator.includes(marker)) failures.push(`scripts/generate-tracked-links.mjs is missing marker: ${marker}`);
  }
}

if (existsSync("scripts/generate-platform-playbook.mjs")) {
  const playbookGenerator = read("scripts/generate-platform-playbook.mjs");
  for (const marker of ["content-automation/PLATFORM_PLAYBOOK.md", "platform-rules.json", "Ready-copy file", "Publishing QA", "Generated platform playbook"]) {
    if (!playbookGenerator.includes(marker)) failures.push(`scripts/generate-platform-playbook.mjs is missing marker: ${marker}`);
  }
}

if (existsSync("scripts/generate-platform-posting-qa.mjs")) {
  const postingQaGenerator = read("scripts/generate-platform-posting-qa.mjs");
  for (const marker of ["content-automation/PLATFORM_POSTING_QA.md", "content-automation/TRACKED_LINKS.csv", "platform-rules.json", "Campaign Posting Matrix", "Editor Paste QA", "Generated platform posting QA"]) {
    if (!postingQaGenerator.includes(marker)) failures.push(`scripts/generate-platform-posting-qa.mjs is missing marker: ${marker}`);
  }
}

if (existsSync("scripts/generate-platform-fit-report.mjs")) {
...
if (existsSync("scripts/generate-korean-readability-report.mjs")) {
  ...
  for (const marker of ["content-automation/KOREAN_READABILITY_REPORT.md", "Readability Matrix", "Hangul chars", "Longest paragraph", "long paragraph over", "suspected mojibake", "Generated Korean readability report"]) {
...
if (existsSync("scripts/generate-ready-copy-index.mjs")) {
  const readyIndexGenerator = read("scripts/generate-ready-copy-index.mjs");
  for (const marker of ["content-automation/READY_COPY_INDEX.md", "Campaign Ready Copy", "Naver Plain Text", "Generated ready copy index"]) {
    if (!readyIndexGenerator.includes(marker)) failures.push(`scripts/generate-ready-copy-index.mjs is missing marker: ${marker}`);
  }
}

if (existsSync("scripts/generate-copy-quality-report.mjs")) {
  const qualityGenerator = read("scripts/generate-copy-quality-report.mjs");
  for (const marker of ["content-automation/COPY_QUALITY_REPORT.md", "Readable, URL-forward, and platform-aware", "Needs review", "Generated copy quality report"]) {
    if (!qualityGenerator.includes(marker)) failures.push(`scripts/generate-copy-quality-report.mjs is missing marker: ${marker}`);
  }
}

if (existsSync("scripts/generate-daily-brief.mjs")) {
  const briefGenerator = read("scripts/generate-daily-brief.mjs");
  for (const marker of ["content-automation/DAILY_BRIEF.md", "Today Focus", "Tomorrow Prep", "Update These Records", "Asia/Seoul", "Generated daily brief"]) {
    if (!briefGenerator.includes(marker)) failures.push(`scripts/generate-daily-brief.mjs is missing marker: ${marker}`);
  }
}
...
if (existsSync("scripts/run-all-content-automation.mjs")) {
  ...
  for (const marker of ["content-automation/campaigns", "run-content-automation.mjs", "generate-content-status.mjs", "generate-publishing-calendar.mjs", "generate-today-actions.mjs", "generate-upcoming-actions.mjs", "generate-tracked-links.mjs", "generate-performance-log.mjs", "generate-platform-playbook.mjs", "generate-platform-posting-qa.mjs", "generate-ready-copy-index.mjs", "generate-copy-quality-report.mjs", "generate-daily-brief.mjs", "Content automation completed for", "campaignFiles.length"]) {
...
if (existsSync("content-automation/UPCOMING_ACTIONS.md")) {
  const upcomingActions = read("content-automation/UPCOMING_ACTIONS.md");
  for (const marker of ["Grant Labs Upcoming Actions", "Upcoming Queue", "Date Summary", "Preparation Log", "Source Files", "npm run content:upcoming", "consultation-checklist-conversion"]) {
    if (!upcomingActions.includes(marker)) failures.push(`content-automation/UPCOMING_ACTIONS.md is missing marker: ${marker}`);
  }
}
...
if (existsSync("content-automation/TRACKED_LINKS.md")) {
  ...
  for (const marker of ["Grant Labs Tracked Links", "utm_source", "utm_medium", "utm_campaign", "utm_content", "npm run content:links", "Naver Blog", "LinkedIn Page"]) {
...
if (existsSync("content-automation/PLATFORM_PLAYBOOK.md")) {
  ...
  for (const marker of ["Grant Labs Platform Playbook", "How To Use", "Publishing QA", "Ready-copy file", "TRACKED_LINKS.md", "PERFORMANCE_LOG.md", "Naver Blog", "LinkedIn Page"]) {
    if (!playbook.includes(marker)) failures.push(`content-automation/PLATFORM_PLAYBOOK.md is missing marker: ${marker}`);
  }
}

if (existsSync("content-automation/PLATFORM_POSTING_QA.md")) {
  const postingQa = read("content-automation/PLATFORM_POSTING_QA.md");
  for (const marker of ["Grant Labs Platform Posting QA", "Campaign Posting Matrix", "Platform Editor QA", "Tracked CTA URL", "Link handling", "Editor Paste QA", "npm run content:posting-qa", "Naver Blog", "Instagram Reels", "YouTube Shorts"]) {
    if (!postingQa.includes(marker)) failures.push(`content-automation/PLATFORM_POSTING_QA.md is missing marker: ${marker}`);
  }
}

if (existsSync("content-automation/PLATFORM_FIT_REPORT.md")) {
...
if (existsSync("content-automation/KOREAN_READABILITY_REPORT.md")) {
  ...
  for (const marker of ["Grant Labs Korean Readability Report", "Readability Matrix", "Hangul chars", "Longest paragraph", "Raw URL", "Posting Rule", "npm run content:readability"]) {
    if (!readability.includes(marker)) failures.push(`content-automation/KOREAN_READABILITY_REPORT.md is missing marker: ${marker}`);
  }
}

if (existsSync("content-automation/READY_COPY_INDEX.md")) {
  const readyIndex = read("content-automation/READY_COPY_INDEX.md");
  for (const marker of ["Grant Labs Ready Copy Index", "Campaign Ready Copy", "Naver Plain Text", "Platform File Map", "npm run content:ready-index"]) {
    if (!readyIndex.includes(marker)) failures.push(`content-automation/READY_COPY_INDEX.md is missing marker: ${marker}`);
  }
}

if (existsSync("content-automation/COPY_QUALITY_REPORT.md")) {
  const quality = read("content-automation/COPY_QUALITY_REPORT.md");
  for (const marker of ["Grant Labs Copy Quality Report", "Ready: 32", "Needs review or missing: 0", "Readable, URL-forward, and platform-aware", "npm run content:quality"]) {
    if (!quality.includes(marker)) failures.push(`content-automation/COPY_QUALITY_REPORT.md is missing marker: ${marker}`);
  }
}

if (existsSync("content-automation/DAILY_BRIEF.md")) {
  const dailyBrief = read("content-automation/DAILY_BRIEF.md");
  for (const marker of ["Grant Labs Daily Brief", "Snapshot", "Today Focus", "Tomorrow Prep", "Update These Records", "npm run content:brief"]) {
    if (!dailyBrief.includes(marker)) failures.push(`content-automation/DAILY_BRIEF.md is missing marker: ${marker}`);
  }
}
...
if (existsSync(".github/workflows/static-site-check.yml")) {
  ...
  for (const marker of ["actions/setup-node@v4", "fetch-depth: 0", "node-version: 20", "timeout-minutes: 5", "concurrency:", "cancel-in-progress: true", "npm run ops:refresh", "git diff --exit-code -- content-automation", "git diff --exit-code -- sitemap.xml", "git diff --exit-code -- DEPLOYMENT_READINESS.md", "git diff --exit-code -- DEVELOPMENT_JOURNAL.md", "git diff --exit-code -- STATUS_INDEX.md", "git diff --exit-code -- OPS_REFRESH_REPORT.md", "npm run serve", "npm run preview:check", "127.0.0.1:4173"]) {
