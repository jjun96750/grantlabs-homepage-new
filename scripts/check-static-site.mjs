...
if (existsSync("scripts/generate-today-actions.mjs")) {
  ...
  for (const marker of ["content-automation/TODAY_ACTIONS.md", "content-automation/campaigns", "content-automation/platform-rules.json", "Source Files", "Platform Execution Notes", "Reporting Log", "Asia/Seoul", "Today Queue", "Generated today actions"]) {
...
if (existsSync("content-automation/TODAY_ACTIONS.md")) {
  ...
  for (const marker of ["Grant Labs Today Actions", "Today Queue", "Source Files", "Platform Execution Notes", "Reporting Log", "Published URL", "npm run content:today", "consultation-checklist-conversion"]) {
...
if (existsSync("COMMANDS.md")) {
  ...
  for (const marker of ["npm run check", "npm run check:content", "npm run content:assets", "npm run content:captions", "npm run content:plan", "npm run content:queue", "npm run content:calendar", "npm run content:today", "npm run content:upcoming", "npm run content:performance", "npm run content:links", "npm run content:playbook", "npm run content:quality", "npm run content:brief", "npm run content:run", "npm run content:run:all", "npm run content:status", "npm run deployment:readiness", "npm run sitemap:refresh", "npm run status:journal", "npm run status:index", "npm run serve", "npm run preview:check", "npm run smoke", "static-site validation", "content automation quality", "asset briefs", "caption pack", "platform-specific posting guidance", "publishing queue", "publishing calendar", "today actions", "upcoming actions", "performance log", "tracked links", "platform playbook", "copy quality report", "daily brief", "full content automation", "all campaigns", "campaign status", "deployment readiness", "sitemap refresh", "development journal", "status index"]) {
