...
- Main branch: `main`
- Latest pushed commit at handoff update: `51f6819 Validate EmailJS configuration`
...
- `scripts/check-static-site.mjs` verifies required files, metadata, duplicate HTML ids, hero image performance attributes, contact form required fields, canonical contact values, EmailJS config values, JSON-LD, docs markers, editor config, Node version pin, gitignore hygiene, SVG asset metadata, robots/sitemap/canonical consistency, local `href` / `src` targets, cross-page hashes, and external new-window link safety.
